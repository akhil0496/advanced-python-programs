...
var_discount_2 = 600                     #Second discount starts.
var_discount_3 = 800                     #Third discount starts.
...
def def_default():
    ...
    list_item_order = [0] * 100                    #Create a list, length 100. Max index number is 99.if needed can add upto 100.
def_default()                                      #Index: 0-10 for foods, index: 10-20 for drinks,
                                                   #Index: from 20 for other services. Global variables.
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_drinks):
        list_item_price[10 + i] = float(list_drinks[i][int(list_drinks[i].index("Rs") + 3):]) # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[20 + i] = float(list_services[i][int(list_services[i].index("Rs") + 3):]) # Applying extraction on 80 and above items wich are Services
        i += 1
def_file_sorter()
    ...
    while True:
            ...
            while i < len(list_foods) or i < len(list_drinks):
                ...
                if i < len(list_drinks):
                    drink = "(" + str(11 + i) + ")" + " " + str(list_drinks[i])
                else:
                    drink = ""
                print(food, drink)
                i += 1
            ...
            try:        #Cautions Error Handling to prevent program crashing and hand out exceptions as a readable error to notify user
                int(input_1)
                if ((int(input_1) <= len(list_foods) and int(input_1) > 0) or (int(input_1) <= len(list_drinks) + 10 and int(input_1) > 10)):
                     try:
                        ...
                        pass
                     try:
                         print("\n" + "_" * 72 + "\n" + str(list_drinks[int(input_1) - 10])) # Handling Drinks Selection / The try/Execpt to handle out of index error as if it  not exists in the array
                     except:
                        pass
    ...
    while True:
        ...
        while i < len(list_item_order): #Enumarating order array items and summing up its prices * quantities
            if(list_item_order[i] != 0):
                ...
                if (i >= 10) and (i < 20):
                    report_new += "\n" + " " * 17 + str(list_drinks[i - 10]) + "  x  " + str(list_item_order[i])
                    print(" " * 17 + str(list_drinks[i - 10]) + "   x  " + str(list_item_order[i]))
                    total_price += list_item_price[i] * list_item_order[i] # Calculating the total price for drinks
                if (i >= 20) and (i < 30):
                    report_new += "\n" + " " * 17 + str(list_services[i - 30])
                    print(" " * 17 + str(list_services[i - 30]))
